...
public class DragNDropCursorAdapter extends SimpleCursorAdapter implements DragNDropAdapter {
    ...
    public DragNDropCursorAdapter(Context context, @IdRes int layout, Cursor cursor, String[] from,
                                  int[] to, @IdRes int handler) {
    ...
    public void onItemDrop(DragNDropListView parent, View view, int startIndex, int endIndex, long id) {
        int position = mPosition[startIndex];

        if (startIndex < endIndex) {
            System.arraycopy(mPosition, startIndex + 1, mPosition, startIndex, endIndex - startIndex);
        } else if (endIndex < startIndex) {
            System.arraycopy(mPosition, endIndex, mPosition, endIndex + 1, startIndex - endIndex);
        }

        mPosition[endIndex] = position;
    }
